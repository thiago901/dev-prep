...
- **Decision:** one activity for every take-a-side format. `interactionMode` decides how it is pressed — `binary` for true/false statements, `buttons` for would/would not, `swipe` for the recessed card that drags with the pointer and rotates with travel, side legends lighting only in the direction of travel. The two keys always do exactly what the drag does; nothing here is gesture-only. The verdict panel pairs "sua decisão" with "decisão esperada" and a match lamp, then the reasoning, the context and the trade-off, inside a polite live region.
...
- **Answer rubric:** four rows on one faceplate — misses the point (record ink), works but incomplete (brass), strong (monitor), interview-ready (legend). It is how an open answer is judged, and it sits on the answer side with the model answers.
- **Sources:** a list of the documentation an item cites, as rows with the title in body ink and the publisher as a legend, each opening in a new tab. It renders on the answer side, and inline on a learn card, which has no gate.
